...
Resolution status: all listed issues were implemented and regression-tested on 2026-09-03. The original observations and proposed solutions remain below as a record of the dogfood investigation.
...
Status: fixed with version-2 manifests using SHA-256 digests of normalized original source slices.
...
- Add a cross-runtime round-trip test in which one runtime writes a manifest and the other reports zero changes.
...
Status: fixed with matching mutation-safe coverage/test profiles, explicit custom coverage policy, and LCOV provenance validation.
...
Status: fixed with global `Mnnn` labels, deterministic form/path/rule identities, exact locations, and `--mutation` selection.
...
Status: fixed with trusted manifest provenance and a planning short circuit before coverage, baseline, or worker setup.
...
Status: fixed with branch-aware `if-not` matching, symmetric `>`/`>=` trim suppression, and explicit positive and negative specs.
